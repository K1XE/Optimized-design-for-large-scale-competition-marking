...
% 蒙特卡洛
for m = 1 :1e3
    gap = randi(121,[124,1]); % 每组卷随机连续五名专家批阅
    res_gap = zeros(3000,125); 
%     res_gap(1,1:5) = 1;
    rand_idx = randi(125,[3000,5]);
    cnt=0;
    for i = 1:length(rand_idx)
%         res_gap(i+1,:) = [zeros(1,gap(i)-1) ones(1,5) zeros(1,121-gap(i))];
        res_gap(i,:) = zeros(1,125);
        cnt = cnt+1;
...
plot(1:125,best_s,'b-','LineWidth',1.4);
hold on 
xlim([0,125.5]);
xlabel('专家序号');
ylabel('评阅试卷集（份）');
